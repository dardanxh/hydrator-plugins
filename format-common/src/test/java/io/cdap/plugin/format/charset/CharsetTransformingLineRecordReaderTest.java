...
public class CharsetTransformingLineRecordReaderTest {
  ...
  public void testGetNextLine() throws IOException {
    ...
    Assert.assertEquals(recordReader.getCurrentValue().toString(), "abcd");
    //Ensure the second line is read
    Assert.assertTrue(recordReader.nextKeyValue());
    Assert.assertEquals(recordReader.getCurrentKey().get(), 5);
    Assert.assertEquals(recordReader.getCurrentValue().toString(), "edfg");
